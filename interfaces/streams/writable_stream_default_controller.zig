...
/// WritableStreamDefaultController zoop class
/// 
/// IDL:
/// ```webidl
/// [Exposed=*]
...
pub const WritableStreamDefaultController = struct {
    ...
    abort_algorithm: common.AbortAlgorithm,
    /// [[closeAlgorithm]]: Promise-returning algorithm for close
    close_algorithm: common.CloseAlgorithm,
    ...
    write_algorithm: common.WriteAlgorithm,
    ...
    transform_controller: ?*anyopaque,
    ...
    pub fn init(
        ...
        return .{
            .allocator = allocator,
            .abort_algorithm = abort_algorithm,
            .close_algorithm = close_algorithm,
    ...
    fn clearAlgorithms(self: *WritableStreamDefaultController) void {
        ...
        self.abort_algorithm = common.defaultAbortAlgorithm();
        self.close_algorithm = common.defaultCloseAlgorithm();
        self.write_algorithm = common.defaultWriteAlgorithm();
    ...
    pub fn abortSteps(self: *WritableStreamDefaultController, reason: ?common.JSValue) !*AsyncPromise(void) {
        ...
        const result = self.abort_algorithm.call(reason);
    ...
    pub fn writeChunk(self: *WritableStreamDefaultController, chunk: common.JSValue, stream: *anyopaque) void {
        ...
        if (self.transform_controller) |transform_ctrl_ptr| {
        ...
        const chunk_size = self.strategy_size_algorithm.call(chunk);

        // Step 3: Call write algorithm (currently sync, wrapping in async)
        const write_promise = self.write_algorithm.call(chunk);
        ...
        const onFulfilled = struct {
            fn call(ctx: *anyopaque, _: void) !void {
                ...
                // Update queue size
                c.controller.queue.queue_total_size += c.chunk_size;
                ...
                if (ws.write_requests.items.len > 0) {
                    const write_req = ws.write_requests.orderedRemove(0);
                    write_req.fulfill({});
    ...
    pub fn processWrite(self: *WritableStreamDefaultController, chunk: common.JSValue) void {
        ...
        const sink_write_promise = self.write_algorithm.call(chunk);
    ...
    pub fn processClose(self: *WritableStreamDefaultController) void {
        ...
        const sink_close_promise = self.close_algorithm.call();
