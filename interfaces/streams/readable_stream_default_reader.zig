...
const readable_stream_mod = @import("readable_stream");
const ReadableStream = readable_stream_mod.ReadableStream;
/// ReadableStreamDefaultReader zoop class
/// 
/// IDL:
/// ```webidl
/// [Exposed=*]
...
pub const ReadableStreamDefaultReader = struct {
    ...
    pub fn deinit(self: *ReadableStreamDefaultReader) void {
        // Promises (closed_promise) are allocated from the event loop's arena.
        ...
        // Note: read_requests promises are owned by callers, just clear the list
    ...
    fn readInternal(self: *ReadableStreamDefaultReader) !*AsyncPromise(common.ReadResult) {
        ...
        switch (stream.state) {
            ...
            .errored => {
                ...
                promise.reject(stream.stored_error orelse common.JSValue.undefined_value());
                return promise;
            },
            .readable => {
                ...
                if (!stream.controller.queue.isEmpty()) {
                    ...
                    if (stream.controller.close_requested and stream.controller.queue.isEmpty()) {
                    ...
                    try self.read_requests.append(self.allocator, promise);
    ...
    fn genericRelease(self: *ReadableStreamDefaultReader) void {
        ...
        if (self.stream) |stream| {
            ...
            // NOTE: We only call deinit() to free the read_requests ArrayList.
...
test "Async read - multiple pending reads fulfilled in order" {
    ...
    try std.testing.expectEqual(@as(usize, 3), reader.read_requests.items.len);
    ...
    try std.testing.expect(read2.isPending());
    try std.testing.expect(read3.isPending());
    try std.testing.expectEqual(@as(usize, 2), reader.read_requests.items.len);
