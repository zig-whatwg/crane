...
/// - Avoids UTF-8 ↔ UTF-16 conversion overhead for Zig consumers
/// - TextDecoder output is typically consumed as UTF-8 in Zig
///
/// **For JavaScript bindings**:
/// Use the conversion helpers below to bridge Zig UTF-8 ↔ WebIDL UTF-16:
...
/// ✅ WebIDL spec compliance (for JavaScript bindings)
/// ✅ Zig ergonomics (UTF-8 native APIs)
/// ✅ Performance (no conversion overhead for Zig-only code)
...
///
/// WHATWG Encoding Standard §5.1.3 (TextDecoder constructor)
/// WHATWG Encoding Standard §5.1.4 (TextDecoder.decode())
///
/// Error Mapping per WHATWG spec:
/// - InvalidEncoding → RangeError (WebIDL simple exception)
/// - ReplacementEncoding → RangeError (WebIDL simple exception)
/// - DecodingError → TypeError (WebIDL simple exception)
///
...
pub const TextDecoder = struct {
    ...
    bom_seen: bool,
    encoding_name: webidl.DOMString,
    ...
    /// 
    /// Errors:
    /// - error.InvalidEncoding → WebIDL RangeError (invalid encoding label)
    /// - error.ReplacementEncoding → WebIDL RangeError (replacement encoding)
    pub fn init(
        allocator: std.mem.Allocator,
        ...
        // Step 2: Get encoding from label (§4.2 get an encoding)
        ...
        const encoding_name = try infra.string.utf8ToUtf16(allocator, enc.whatwg_name);
        errdefer allocator.free(encoding_name);
    ...
    pub fn deinit(self: *TextDecoder) void {
        ...
        self.allocator.free(self.encoding_name);
    ...
    /// - error.DetachedBuffer → WebIDL TypeError (detached ArrayBuffer)
    pub fn call_decode(
        self: *TextDecoder,
    ...
    fn decodeBytes(
        ...
        // WHATWG Encoding Standard § 7.5 (decode algorithm, step 1)
        // https://encoding.spec.whatwg.org/#dom-textdecoder-decode
        ...
        // Step 2: Set do_not_flush to options[stream]
        ...
        if (!self.bom_seen and ascii_mod.isAscii(bytes)) {
            // ASCII → UTF-16: each byte becomes a u16
        ...
        if (!self._ignoreBOM and !self.bom_seen) {
        ...
        const output_buf = blk: {
            if (self.reusable_utf16_buf) |buf| {
                ...
                self.reusable_utf16_buf = try self.allocator.realloc(buf, max_output_len);
                break :blk self.reusable_utf16_buf.?;
            }
            // First allocation
            self.reusable_utf16_buf = try self.allocator.alloc(u16, max_output_len);
            break :blk self.reusable_utf16_buf.?;
        };
        // No defer - we keep the buffer for reuse!

        // Decode to UTF-16
        const result = decoder.decode(bytes, output_buf, !self.do_not_flush);
...
test "TextDecoder - basic UTF-8 decoding" {
    ...
    var array_buffer = try webidl.ArrayBuffer.init(allocator, input.len);
    defer array_buffer.deinit(allocator);
    @memcpy(array_buffer.data, input);

    const buffer_source = webidl.AllowSharedBufferSource{ .array_buffer = &array_buffer };
...
test "TextDecoder - WebIDL decode with non-ASCII UTF-8" {
    ...
    // UTF-8 input: "Hello, 世界" (Chinese "world")
...
test "TextDecoder - WebIDL decode with surrogate pair" {
    ...
    // UTF-8 input: "😀" (U+1F600 GRINNING FACE - requires surrogate pair in UTF-16)
    const input = "😀";
    var array_buffer = try webidl.ArrayBuffer.init(allocator, input.len);
    defer array_buffer.deinit(allocator);
    @memcpy(array_buffer.data, input);
...
test "TextDecoder - BOM stripping basic" {
    ...
    var array_buffer = try webidl.ArrayBuffer.init(allocator, input_with_bom.len);
    defer array_buffer.deinit(allocator);
    @memcpy(array_buffer.data, &input_with_bom);
...
test "TextDecoder - BOM reset on stream end" {
    ...
    // This sets do_not_flush=false, which triggers reset on next call
    {
        var array_buffer = try webidl.ArrayBuffer.init(allocator, 0);
        defer array_buffer.deinit(allocator);
    ...
    // Per WHATWG spec: do_not_flush is now false, so step 1 resets bom_seen
    {
        var array_buffer = try webidl.ArrayBuffer.init(allocator, input_with_bom.len);
        defer array_buffer.deinit(allocator);
        @memcpy(array_buffer.data, &input_with_bom);
...
test "TextDecoder - decode with AllowSharedBufferSource (ArrayBuffer)" {
    ...
    var array_buffer = try webidl.ArrayBuffer.init(allocator, 13);
    defer array_buffer.deinit(allocator);

    @memcpy(array_buffer.data, "Hello, World!");
...
test "TextDecoder - decode with AllowSharedBufferSource (Uint8Array)" {
    ...
    var array_buffer = try webidl.ArrayBuffer.init(allocator, 5);
    defer array_buffer.deinit(allocator);

    @memcpy(array_buffer.data, "Hello");

    const typed_array = try webidl.TypedArray(u8).init(&array_buffer, 0, 5);
    const array_buffer_view = webidl.ArrayBufferView{ .uint8_array = typed_array };
    const buffer_source = webidl.AllowSharedBufferSource{ .array_buffer_view = array_buffer_view };
...
test "TextDecoder - decode with AllowSharedBufferSource (SharedArrayBuffer)" {
    ...
    const buffer_source = webidl.AllowSharedBufferSource{ .shared_array_buffer = &shared_buffer };
    const output = try decoder.call_decode(buffer_source, .{});
    defer allocator.free(output);
...
test "TextDecoder - decode with AllowSharedBufferSource (DataView)" {
    ...
    var array_buffer = try webidl.ArrayBuffer.init(allocator, 4);
    defer array_buffer.deinit(allocator);

    @memcpy(array_buffer.data, "Test");

    const data_view = try webidl.DataView.init(&array_buffer, 0, 4);
    const array_buffer_view = webidl.ArrayBufferView{ .data_view = data_view };
    const buffer_source = webidl.AllowSharedBufferSource{ .array_buffer_view = array_buffer_view };
