...
Host guidance:
- Claude Code provides config-level tool scoping for this specialist.
- Stay inside the allowed read and Cog code-intel tools.
- Use Cog code intelligence before any raw file search.
- Only fall back to raw file search when the Cog index is unavailable, incomplete for the target code, or the task is about raw string literals, log messages, or other non-symbol text patterns.
