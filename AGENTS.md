...
**Files in `generated/` are code-generated outputs. NEVER make direct changes to them unless explicitly directed by the user.**

**General Rule:**
- All files in `generated/` are automatically generated by the codegen system
- Changes MUST be made to the codegen source files (in `src/codegen/` or similar)
- After updating codegen, regenerate ALL files with: `zig build codegen -- idl/ --dest-root generated/ --impls`
...
2. ⚠️ Once the fix is working, you MUST:
   ...
   - Regenerate ALL generated files: `zig build codegen -- idl/ --dest-root generated/ --impls`
...
- The `generated/` directory should be committed to version control
- Generated files change when codegen logic changes or IDL files are updated

**Codegen Command:**
```bash
zig build codegen -- idl/ --dest-root generated/ --impls
```
...
4. Run: `zig build codegen -- idl/ --dest-root generated/ --impls`
5. Verify tests pass
...
├── PostMessageOptions.idl   # Additional type definitions for testing
└── XRFeatureInit.idl        # Supplementary types not in main webref
...
**Date**: 2025-11-22  
**Lesson**: When debugging codegen issues, always delete `generated/` and regenerate completely from scratch.

**Why**: Partial regeneration can mask systemic issues because:
- Impl files are protected from overwriting (line 570 in generator.zig: `std.fs.cwd().access(output_path, .{}) catch`)
...
1. Delete `generated/` completely
2. Regenerate with `zig build codegen -- idl/ --dest-root generated/ --impls`
...
Agent: "I want to first try to regenerate all of the files including impls,
        delete the generated/ directory first then see if the problem still exists"
```
