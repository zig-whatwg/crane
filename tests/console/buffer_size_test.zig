...
test "custom buffer size - small (embedded)" {
    const allocator = std.testing.allocator;
    var console_obj = try Console.initWithBufferSize(allocator, 10);
    defer console_obj.deinit();
...
test "custom buffer size - large (server)" {
    const allocator = std.testing.allocator;
    var console_obj = try Console.initWithBufferSize(allocator, 5000);
    defer console_obj.deinit();
...
test "zero buffer size - no buffering (testing mode)" {
    const allocator = std.testing.allocator;
    var console_obj = try Console.initWithBufferSize(allocator, 0);
    defer console_obj.deinit();
...
test "buffer size 1 - only keeps most recent message" {
    const allocator = std.testing.allocator;
    var console_obj = try Console.initWithBufferSize(allocator, 1);
    defer console_obj.deinit();
...
test "buffer overflow behavior - FIFO (oldest discarded)" {
    const allocator = std.testing.allocator;
    var console_obj = try Console.initWithBufferSize(allocator, 3);
    defer console_obj.deinit();
...
test "multiple consoles with different buffer sizes" {
    ...
    var console_small = try Console.initWithBufferSize(allocator, 5);
    defer console_small.deinit();

    var console_large = try Console.initWithBufferSize(allocator, 100);
