...
test "default buffer size is 1000" {
    ...
    try std.testing.expectEqual(@as(usize, 1000), console_obj.messageBuffer.maxSize);
}
...
test "custom buffer size - small (embedded)" {
    ...
    try std.testing.expectEqual(@as(usize, 10), console_obj.messageBuffer.maxSize);
    ...
    try std.testing.expectEqual(@as(usize, 10), console_obj.messageBuffer.size());
...
test "custom buffer size - large (server)" {
    ...
    try std.testing.expectEqual(@as(usize, 5000), console_obj.messageBuffer.maxSize);
    ...
    try std.testing.expectEqual(@as(usize, 100), console_obj.messageBuffer.size());
    try std.testing.expect(!console_obj.messageBuffer.isFull());
}
...
test "zero buffer size - no buffering (testing mode)" {
    ...
    try std.testing.expectEqual(@as(usize, 0), console_obj.messageBuffer.maxSize);
...
test "buffer size 1 - only keeps most recent message" {
    ...
    try std.testing.expectEqual(@as(usize, 1), console_obj.messageBuffer.maxSize);
...
test "multiple consoles with different buffer sizes" {
    ...
    try std.testing.expectEqual(@as(usize, 5), console_small.messageBuffer.maxSize);
    try std.testing.expectEqual(@as(usize, 100), console_large.messageBuffer.maxSize);
    ...
    try std.testing.expectEqual(@as(usize, 1), console_small.messageBuffer.size());
    try std.testing.expectEqual(@as(usize, 1), console_large.messageBuffer.size());
}
