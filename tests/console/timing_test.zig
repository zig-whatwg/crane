...
test "timing - multiple concurrent timers" {
    ...
    // End timer2
    console_obj.call_timeEnd(timer2);
    ...
    console_obj.call_timeEnd(timer3);
    try std.testing.expect(console_obj.timerTable.isEmpty());
}
...
test "timing - timer labels are independent" {
    ...
    // Clean up
    console_obj.call_timeEnd(label_a);
    console_obj.call_timeEnd(label_b);
}
...
test "timing - default label can be used" {
    ...
    console_obj.call_time(default_label);
    try std.testing.expect(console_obj.timerTable.contains(default_label));

    console_obj.call_timeLog(default_label, &.{});
    try std.testing.expect(console_obj.timerTable.contains(default_label));

    console_obj.call_timeEnd(default_label);
    try std.testing.expect(!console_obj.timerTable.contains(default_label));
}
...
test "timing - memory safety with many timers" {
    ...
    var i: usize = 0;
    while (i < 50) : (i += 1) {
        const label = try std.fmt.allocPrint(allocator, "timer{d}", .{i});
