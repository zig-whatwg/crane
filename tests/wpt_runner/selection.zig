...
pub fn parseWorklist(allocator: Allocator, bytes: []const u8) !Worklist {
    ...
    while (lines.next()) |raw| {
        ...
        // `#` comments, so a checked-in worklist can say where it came from.
        // Dropping them here does not disturb the index space the journal
        // resumes against: parent and child parse the same bytes the same way,
        // so both arrive at the same positions. No WPT path begins with `#`.
        if (line[0] == '#') continue;
...
test "parseWorklist ignores # comments" {
    const allocator = std.testing.allocator;

    // tests/wpt_0_1_worklist.txt is generated and carries a header saying so,
    // because a bare path list with no provenance rots into folklore. Without
    // this, those header lines parse as test paths and the run starts with four
    // bogus entries.
    var wl = try parseWorklist(allocator,
        \\# Crane 0.1 WPT worklist - GENERATED by tools/wpt_subset.py
        \\# Do not hand-edit.
        \\dom/a.html
    ...
    try std.testing.expectEqual(@as(usize, 2), wl.paths.len);
    try std.testing.expectEqualStrings("dom/a.html", wl.paths[0]);
    try std.testing.expectEqualStrings("html/b.html", wl.paths[1]);
}
