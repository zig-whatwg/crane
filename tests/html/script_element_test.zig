...
test "ScriptRunner - initialization" {
    ...
    var runner = html.ScriptRunner.init(allocator);
    defer runner.deinit();
...
test "ScriptRunner - queue deferred script" {
    ...
    // Create a mock script (using pointer from int as placeholder)
    const mock_script: *runtime.Instance = @ptrFromInt(0x1234);
...
test "ScriptRunner - multiple deferred scripts in order" {
    ...
    // Queue multiple deferred scripts
    const script1: *runtime.Instance = @ptrFromInt(0x1001);
    const script2: *runtime.Instance = @ptrFromInt(0x1002);
    const script3: *runtime.Instance = @ptrFromInt(0x1003);
