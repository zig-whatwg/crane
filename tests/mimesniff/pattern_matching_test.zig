...
fn expectEssence(mime_type: source.MimeType, expected_type: []const u8, expected_subtype: []const u8) !void {
...
test "matchesMp4Signature - valid ftyp box" {
    ...
    try std.testing.expect(source.matchesMp4Signature(&mp4_signature));
}
...
test "matchesMp4Signature - invalid box type" {
    ...
    try std.testing.expect(!source.matchesMp4Signature(&not_ftyp));
}
test "matchesWebmSignature - valid EBML with DocType webm" {
    ...
    try std.testing.expect(source.matchesWebmSignature(&webm_signature));
}
test "matchesWebmSignature - matroska does not match" {
    ...
    try std.testing.expect(!source.matchesWebmSignature(&matroska_signature));
}
test "matchesWebmSignature - invalid DocType" {
    ...
    try std.testing.expect(!source.matchesWebmSignature(&invalid_signature));
}
test "matchesMp3Signature - valid MP3 with ID3" {
    ...
    try std.testing.expect(!source.matchesMp3Signature(&mp3_with_id3));
}
test "matchesMp3Signature - valid MP3 without ID3" {
    ...
    try std.testing.expect(source.matchesMp3Signature(mp3_data.items()));
}
test "matchesMp3Signature - invalid MP3 header (bad bitrate)" {
    ...
    try std.testing.expect(!source.matchesMp3Signature(&invalid_mp3));
}
...
test "matchesMp3Signature - too short" {
    ...
    try std.testing.expect(!source.matchesMp3Signature(&short_mp3));
}