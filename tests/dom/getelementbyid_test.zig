...
test "Document.getElementById - case sensitive matching" {
    ...
    const found_lower = doc.call_getElementById(allocator, "myid");
    try std.testing.expect(found_lower == null);

    // Exact match - should match
    const found_exact = doc.call_getElementById(allocator, "MyID");
...
test "Document.getElementById - finds element after id attribute added" {
    ...
    // Initially no id
    const found_before = doc.call_getElementById(allocator, "added-later");
    ...
    // Now should find it
    const found_after = doc.call_getElementById(allocator, "added-later");
...
test "Document.getElementById - does not find element after id attribute removed" {
    ...
    const found_before = doc.call_getElementById(allocator, "will-remove");
    try std.testing.expect(found_before != null);
    ...
    const found_after = doc.call_getElementById(allocator, "will-remove");
    try std.testing.expect(found_after == null);
}
