...
test "TreeWalker - parentNode moves to filtered parent" {
    ...
    // Start at grandchild
    var walker = try TreeWalker.init(allocator, &root, NodeFilter.SHOW_ALL, null);
    defer walker.deinit();
...
test "TreeWalker - whatToShow filters by node type" {
    ...
    // Only show elements
    var walker = try TreeWalker.init(allocator, &root, NodeFilter.SHOW_ELEMENT, null);
    defer walker.deinit();
...
test "TreeWalker - filter callback with FILTER_SKIP descends" {
    ...
    var walker = try TreeWalker.init(allocator, &root, NodeFilter.SHOW_ALL, skipText);
    defer walker.deinit();
...
test "TreeWalker - filter callback with FILTER_REJECT skips subtree" {
    ...
    var walker = try TreeWalker.init(allocator, &root, NodeFilter.SHOW_ALL, rejectText);
    defer walker.deinit();
...
test "TreeWalker - attributes return correct values" {
    ...
    var walker = try TreeWalker.init(allocator, &root, NodeFilter.SHOW_ELEMENT, filter);
    defer walker.deinit();
