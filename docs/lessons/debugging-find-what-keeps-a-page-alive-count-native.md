...
**2026-09-26, again**: the whole-page leak came back - every page kept its context and ~2.3 MB. The heap snapshot showed two handles per old page from "(Global handles)"; site attribution (`tmp/scratch/diag_patch.py` in a throwaway worktree, dumps diffed between the second and third page) named them in one run: the `window`/`self` shortcut returning an undisposed `getThis()`, and the [Global] setter's undisposed `method_ctx` - both copies of code whose main path disposes correctly (f0ada2087). **A shortcut or a second copy of a binding path does not inherit the main path's handle discipline; audit every early return and every duplicated branch.** Measure with three identical frameless pages under `CRANE_HEAP_GC=1`: `native_contexts` must stay flat.
