...
test "windows drive letter - is drive letter" {
    // Valid drive letters with ':'
    try std.testing.expect(isWindowsDriveLetter("c:"));
    try std.testing.expect(isWindowsDriveLetter("C:"));
    try std.testing.expect(isWindowsDriveLetter("z:"));
    try std.testing.expect(isWindowsDriveLetter("A:"));

    // Valid drive letters with '|' (legacy)
    try std.testing.expect(isWindowsDriveLetter("c|"));
    try std.testing.expect(isWindowsDriveLetter("C|"));

    // Invalid - not ASCII alpha
    try std.testing.expect(!isWindowsDriveLetter("1:"));
    try std.testing.expect(!isWindowsDriveLetter("$:"));

    // Invalid - wrong second character
    try std.testing.expect(!isWindowsDriveLetter("c/"));
    try std.testing.expect(!isWindowsDriveLetter("ca"));
...
test "windows drive letter - is normalized" {
    // Normalized drive letters (with ':')
    try std.testing.expect(isNormalizedWindowsDriveLetter("c:"));
    try std.testing.expect(isNormalizedWindowsDriveLetter("C:"));
    try std.testing.expect(isNormalizedWindowsDriveLetter("z:"));
    ...
    // Invalid
    try std.testing.expect(!isNormalizedWindowsDriveLetter("1:"));
    try std.testing.expect(!isNormalizedWindowsDriveLetter("c/"));
    try std.testing.expect(!isNormalizedWindowsDriveLetter("c"));
}

test "windows drive letter - starts with" {
    // Starts with drive letter (length == 2)
    try std.testing.expect(startsWithWindowsDriveLetter("c:"));
    try std.testing.expect(startsWithWindowsDriveLetter("C:"));

    // Starts with drive letter (third is separator)
    try std.testing.expect(startsWithWindowsDriveLetter("c:/"));
    try std.testing.expect(startsWithWindowsDriveLetter("c:\\"));
    try std.testing.expect(startsWithWindowsDriveLetter("c:?"));
    try std.testing.expect(startsWithWindowsDriveLetter("c:#"));
    try std.testing.expect(startsWithWindowsDriveLetter("c:/path/to/file"));

    // Does NOT start with drive letter (third is not separator)
    try std.testing.expect(!startsWithWindowsDriveLetter("c:a"));
    try std.testing.expect(!startsWithWindowsDriveLetter("c:x"));

    // Invalid - not a drive letter
    try std.testing.expect(!startsWithWindowsDriveLetter("1:/"));
    try std.testing.expect(!startsWithWindowsDriveLetter("ca/"));

    // Invalid - too short
    try std.testing.expect(!startsWithWindowsDriveLetter("c"));
    try std.testing.expect(!startsWithWindowsDriveLetter(""));
}

test "windows drive letter - spec examples" {
    // From spec line 882-886
    try std.testing.expect(startsWithWindowsDriveLetter("c:")); // ✅
    try std.testing.expect(startsWithWindowsDriveLetter("c:/")); // ✅
    try std.testing.expect(!startsWithWindowsDriveLetter("c:a")); // ❌
}
