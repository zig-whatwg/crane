...
test "percent encoding - encode byte" {
    // Spec line 124 examples
    try std.testing.expectEqualSlices(u8, "%23", &percentEncodeByte(0x23));
    try std.testing.expectEqualSlices(u8, "%7F", &percentEncodeByte(0x7F));
    try std.testing.expectEqualSlices(u8, "%20", &percentEncodeByte(0x20));
    try std.testing.expectEqualSlices(u8, "%00", &percentEncodeByte(0x00));
    try std.testing.expectEqualSlices(u8, "%FF", &percentEncodeByte(0xFF));
}

test "percent encoding - decode bytes" {
    const allocator = std.testing.allocator;

    // Simple decoding
    {
        const decoded = try percentDecode(allocator, "%20");
        defer allocator.free(decoded);
        try std.testing.expectEqualSlices(u8, " ", decoded);
    }

    // Multiple percent-encoded bytes
    {
        const decoded = try percentDecode(allocator, "%25%20");
        defer allocator.free(decoded);
        try std.testing.expectEqualSlices(u8, "% ", decoded);
    }

    // Spec line 226 example: invalid percent sequences preserved
    {
        const decoded = try percentDecode(allocator, "%25%s%1G");
        defer allocator.free(decoded);
        try std.testing.expectEqualSlices(u8, "%%s%1G", decoded);
    }
    ...
    // No encoding
    {
        const decoded = try percentDecode(allocator, "hello");
        defer allocator.free(decoded);
    ...
    {
        const decoded = try percentDecode(allocator, "test%2");
        defer allocator.free(decoded);
        try std.testing.expectEqualSlices(u8, "test%2", decoded);
    }

    // Just %
    {
        const decoded = try percentDecode(allocator, "test%");
        defer allocator.free(decoded);
        try std.testing.expectEqualSlices(u8, "test%", decoded);
    }
}

test "percent encoding - decode string" {
    const allocator = std.testing.allocator;

    // UTF-8 encoded characters
    {
        const decoded = try percentDecodeString(allocator, "%E2%80%BD");
        defer allocator.free(decoded);
        try std.testing.expectEqualSlices(u8, "‽", decoded);
    }
    ...
    {
        const decoded = try percentDecodeString(allocator, "Say%20what%E2%80%BD");
        defer allocator.free(decoded);
        try std.testing.expectEqualSlices(u8, "Say what‽", decoded);
    }
}

test "percent encoding - UTF-8 encode" {
    const allocator = std.testing.allocator;

    // Simple ASCII with space (spec line 235 example)
    {
        const encoded = try utf8PercentEncode(allocator, "Say what‽", .userinfo);
        defer allocator.free(encoded);
        try std.testing.expectEqualSlices(u8, "Say%20what%E2%80%BD", encoded);
    }

    // Single non-ASCII character (spec line 233)
    {
        const encoded = try utf8PercentEncode(allocator, "≡", .userinfo);
        defer allocator.free(encoded);
        try std.testing.expectEqualSlices(u8, "%E2%89%A1", encoded);
    }

    // Only ASCII, no encoding needed
    {
        const encoded = try utf8PercentEncode(allocator, "hello", .userinfo);
        defer allocator.free(encoded);
        try std.testing.expectEqualSlices(u8, "hello", encoded);
    }

    // Space with different encode sets
    {
        const encoded = try utf8PercentEncode(allocator, "a b", .path);
        defer allocator.free(encoded);
        try std.testing.expectEqualSlices(u8, "a%20b", encoded);
    }

    // Characters in encode set
    {
        const encoded = try utf8PercentEncode(allocator, "a/b", .userinfo);
        defer allocator.free(encoded);
        try std.testing.expectEqualSlices(u8, "a%2Fb", encoded);
    }

    // C0 controls
    {
        const encoded = try utf8PercentEncode(allocator, "a\tb", .c0_control);
        defer allocator.free(encoded);
        try std.testing.expectEqualSlices(u8, "a%09b", encoded);
    }
}
...
test "percent encoding - roundtrip" {
    ...
    const original = "Hello, 世界! 100%";

    // Encode then decode
    const encoded = try utf8PercentEncode(allocator, original, .component);
    defer allocator.free(encoded);

    const decoded = try percentDecodeString(allocator, encoded);
    defer allocator.free(decoded);

    try std.testing.expectEqualSlices(u8, original, decoded);
}

test "percent encoding - encode set differences" {
    const allocator = std.testing.allocator;

    // Fragment includes ` but query doesn't (spec line 162)
    {
        const frag = try utf8PercentEncode(allocator, "`", .fragment);
        defer allocator.free(frag);
        try std.testing.expectEqualSlices(u8, "%60", frag);

        const query = try utf8PercentEncode(allocator, "`", .query);
        defer allocator.free(query);
        try std.testing.expectEqualSlices(u8, "`", query);
    }

    // Path includes ? but query doesn't
    {
        const path_enc = try utf8PercentEncode(allocator, "?", .path);
        defer allocator.free(path_enc);
        try std.testing.expectEqualSlices(u8, "%3F", path_enc);

        const query_enc = try utf8PercentEncode(allocator, "?", .query);
        defer allocator.free(query_enc);
        try std.testing.expectEqualSlices(u8, "?", query_enc);
    }

    // Userinfo includes / but path doesn't
    {
        const userinfo_enc = try utf8PercentEncode(allocator, "/", .userinfo);
        defer allocator.free(userinfo_enc);
        try std.testing.expectEqualSlices(u8, "%2F", userinfo_enc);

        const path_enc = try utf8PercentEncode(allocator, "/", .path);
        defer allocator.free(path_enc);
        try std.testing.expectEqualSlices(u8, "/", path_enc);
    }
}
