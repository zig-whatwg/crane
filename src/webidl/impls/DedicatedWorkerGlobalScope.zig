...
fn dispatchMessageEventDirect(ctx: runtime.Context, serialized_data: *structured_clone.SerializedValue) !void {
    std.log.info("[WORKER] dispatchMessageEventDirect() called", .{});

    // Get the V8 context from runtime context's engine_ctx field
    const engine_ctx = ctx.getEngineContext() orelse {
        std.log.err("[WORKER] dispatchMessageEventDirect() failed to get engine context", .{});
        return error.NoEngineContext;
    };
    const v8_ctx: *v8_engine.ffi.Context = @ptrCast(@alignCast(engine_ctx));

    // Get isolate using V8's current isolate API
    ...
    const global = v8_engine.ffi.v8_Context_Global(v8_ctx) orelse {
        std.log.err("[WORKER] dispatchMessageEventDirect() failed to get global object", .{});
        return error.NoGlobal;
    };

    // Deserialize the data to a V8 value using structured_clone module
    // Note: structuredDeserialize returns a JSValue, not a V8 value directly
    // For now, we'll create a simple V8 value from the serialized primitive
    const deserialized: ?*v8_engine.ffi.Value = switch (serialized_data.*.type) {
        .primitive => blk: {
            const prim = serialized_data.*.data.primitive;
            break :blk switch (prim) {
                .undefined => v8_engine.ffi.v8_Undefined(isolate),
                .null => v8_engine.ffi.v8_Null(isolate),
                .boolean => |b| @ptrCast(v8_engine.ffi.v8_Boolean_New(isolate, b)),
                .number => |n| @ptrCast(v8_engine.ffi.v8_Number_New(isolate, n)),
                .string => |s| @ptrCast(v8_engine.ffi.v8_String_NewFromUtf8(isolate, s.ptr, @intCast(s.len))),
                .bigint => v8_engine.ffi.v8_Undefined(isolate), // TODO: BigInt support
            };
        },
        else => blk: {
            std.log.warn("[WORKER] dispatchMessageEventDirect() unsupported serialized type: {s}", .{@tagName(serialized_data.*.type)});
            break :blk v8_engine.ffi.v8_Undefined(isolate);
        },
    };
    std.log.info("[WORKER] dispatchMessageEventDirect() deserialized data successfully", .{});
    ...
    const onmessage_value = v8_engine.ffi.v8_Object_Get(global, v8_ctx, @ptrCast(onmessage_key)) orelse {
    ...
    std.log.info("[WORKER] dispatchMessageEventDirect() found onmessage handler, creating MessageEvent", .{});

    // Create a MessageEvent-like object with the data property
    const event_obj = v8_engine.ffi.v8_Object_New(isolate) orelse {
        std.log.err("[WORKER] dispatchMessageEventDirect() failed to create event object", .{});
        return error.ObjectCreationFailed;
    };
    const data_key = v8_engine.ffi.v8_String_NewFromUtf8(isolate, "data", 4) orelse {
        std.log.err("[WORKER] dispatchMessageEventDirect() failed to create data key", .{});
        return error.StringCreationFailed;
    };
    _ = v8_engine.ffi.v8_Object_Set(event_obj, v8_ctx, @ptrCast(data_key), deserialized orelse v8_engine.ffi.v8_Undefined(isolate).?);

    // Add type property
    const type_key = v8_engine.ffi.v8_String_NewFromUtf8(isolate, "type", 4) orelse {
        std.log.err("[WORKER] dispatchMessageEventDirect() failed to create type key", .{});
        return error.StringCreationFailed;
    };
    const type_value = v8_engine.ffi.v8_String_NewFromUtf8(isolate, "message", 7) orelse {
        std.log.err("[WORKER] dispatchMessageEventDirect() failed to create type value", .{});
        return error.StringCreationFailed;
    };
    _ = v8_engine.ffi.v8_Object_Set(event_obj, v8_ctx, @ptrCast(type_key), @ptrCast(type_value));

    // Call onmessage(event)
    const onmessage_fn: *v8_engine.ffi.Function = @ptrCast(onmessage_value);
    var args = [_]*v8_engine.ffi.Value{@ptrCast(event_obj)};
    std.log.info("[WORKER] dispatchMessageEventDirect() calling onmessage handler", .{});
    _ = v8_engine.ffi.v8_Function_Call(onmessage_fn, v8_ctx, @ptrCast(global), 1, &args);
