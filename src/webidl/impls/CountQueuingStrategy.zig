...
pub fn call_constructor(ctx: runtime.Context, init_data: dictionaries.QueuingStrategyInit) !*runtime.Instance {
    // Create instance
    const instance = try init(ctx.allocator, State, &CountQueuingStrategy.vtable, ctx);
...
/// Steps (given chunk):
/// 1. Return 1.
fn countSizeFunction(arguments: []const runtime.JSValue) runtime.JSValue {
    _ = arguments;
    // Count strategy always returns 1 per chunk
    return runtime.JSValue.fromNumber(1.0);
}
