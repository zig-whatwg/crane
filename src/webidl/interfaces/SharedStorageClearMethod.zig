...
pub const SharedStorageClearMethod = struct {
    ...
    pub const vtable = runtime.buildVTable(SharedStorageClearMethod, .{
        .deinit_fn = &deinit_wrapper,
    });

    /// Initialize a new instance
    pub fn init(allocator: std.mem.Allocator) !*runtime.Instance {
        return SharedStorageClearMethodImpl.init(allocator, State, &vtable);
    }
    ...
    /// WebIDL constructor
    pub fn call_constructor(allocator: std.mem.Allocator, options: SharedStorageModifierMethodOptions) !*runtime.Instance {
        const instance = try init(allocator);
        errdefer deinit(instance);
        
        try SharedStorageClearMethodImpl.constructor(instance, options);
