...
pub const Node = struct {
    ...
    /// Note: [SameObject] caching is handled in NodeImpl.get_childNodes via internal.child_nodes_list
    /// We can't cache in interface State because subclasses (Element, DocumentFragment, etc.)
    /// may be called through this function but have different State structs.
    pub fn get_childNodes(instance: *runtime.Instance) anyerror!*runtime.Instance {
        return try NodeImpl.get_childNodes(instance);
    }
