...
const XRBodyImpl = @import("../impls/XRBody.zig");
...
pub const XRBody = struct {
    ...
    pub fn get_size(instance: *runtime.Instance) u32 {
        const state = instance.getState(State);
        return XRBodyImpl.get_size(state);
    }

    pub fn call_get(instance: *runtime.Instance, key: anyopaque) anyopaque {
        const state = instance.getState(State);
        
        return XRBodyImpl.call_get(state, key);
    }
