...
const SmartCardErrorImpl = @import("../impls/SmartCardError.zig");
...
pub const SmartCardError = struct {
    ...
    pub const vtable = runtime.buildVTable(SmartCardError, .{
    ...
    pub fn deinit(instance: *runtime.Instance) void {
        ...
        SmartCardErrorImpl.deinit(state);
    }
    ...
    pub fn call_constructor(allocator: std.mem.Allocator, message: runtime.DOMString, options: anyopaque) !*runtime.Instance {
        ...
        const state = instance.getState(State);
        try SmartCardErrorImpl.constructor(state, message, options);
    ...
    pub fn get_name(instance: *runtime.Instance) runtime.DOMString {
        const state = instance.getState(State);
        return SmartCardErrorImpl.get_name(state);
    }

    pub fn get_message(instance: *runtime.Instance) runtime.DOMString {
        const state = instance.getState(State);
        return SmartCardErrorImpl.get_message(state);
    }

    pub fn get_code(instance: *runtime.Instance) u16 {
        const state = instance.getState(State);
        return SmartCardErrorImpl.get_code(state);
    }

    pub fn get_responseCode(instance: *runtime.Instance) anyopaque {
        const state = instance.getState(State);
        return SmartCardErrorImpl.get_responseCode(state);
    }
