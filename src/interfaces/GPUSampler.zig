...
const GPUSamplerImpl = @import("../impls/GPUSampler.zig");
...
pub const GPUSampler = struct {
    ...
    pub const vtable = runtime.buildVTable(GPUSampler, .{
    ...
    pub fn get_label(instance: *runtime.Instance) runtime.USVString {
        const state = instance.getState(State);
        return GPUSamplerImpl.get_label(state);
    }

    pub fn set_label(instance: *runtime.Instance, value: runtime.USVString) void {
        const state = instance.getState(State);
        GPUSamplerImpl.set_label(state, value);
    }
