...
test "PullIntoDescriptor - initialization" {
    ...
    try std.testing.expectEqual(@as(u64, 1024), descriptor.buffer_byte_length);
    try std.testing.expectEqual(@as(u64, 0), descriptor.byte_offset);
    try std.testing.expectEqual(@as(u64, 1024), descriptor.byte_length);
    try std.testing.expectEqual(@as(u64, 0), descriptor.bytes_filled);
    try std.testing.expectEqual(@as(u64, 1), descriptor.minimum_fill);
    try std.testing.expectEqual(@as(u64, 1), descriptor.element_size);
    try std.testing.expectEqual(ViewConstructor.uint8_array, descriptor.view_constructor);
    try std.testing.expectEqual(ReaderType.byob, descriptor.reader_type);
}

test "PullIntoDescriptor - remaining bytes calculation" {
    const allocator = std.testing.allocator;
    ...
    try std.testing.expectEqual(@as(u64, 1024), descriptor.remainingBytes());

    descriptor.bytes_filled = 256;
    try std.testing.expectEqual(@as(u64, 768), descriptor.remainingBytes());

    descriptor.bytes_filled = 1024;
    try std.testing.expectEqual(@as(u64, 0), descriptor.remainingBytes());
}

test "PullIntoDescriptor - minimum fill check" {
    const allocator = std.testing.allocator;
    ...
    try std.testing.expect(!descriptor.isMinimumFillMet());
    ...
    descriptor.bytes_filled = 768;
    try std.testing.expect(descriptor.isMinimumFillMet());
}

test "PullIntoDescriptor - filled check" {
    const allocator = std.testing.allocator;
    ...
    try std.testing.expect(!descriptor.isFilled());
...
test "PullIntoDescriptor - element sizes" {
    try std.testing.expectEqual(@as(u64, 1), PullIntoDescriptor.getElementSize(.data_view));
    try std.testing.expectEqual(@as(u64, 1), PullIntoDescriptor.getElementSize(.uint8_array));
    try std.testing.expectEqual(@as(u64, 1), PullIntoDescriptor.getElementSize(.int8_array));
    try std.testing.expectEqual(@as(u64, 2), PullIntoDescriptor.getElementSize(.uint16_array));
    try std.testing.expectEqual(@as(u64, 2), PullIntoDescriptor.getElementSize(.int16_array));
    try std.testing.expectEqual(@as(u64, 4), PullIntoDescriptor.getElementSize(.uint32_array));
    try std.testing.expectEqual(@as(u64, 4), PullIntoDescriptor.getElementSize(.int32_array));
    try std.testing.expectEqual(@as(u64, 4), PullIntoDescriptor.getElementSize(.float32_array));
    try std.testing.expectEqual(@as(u64, 8), PullIntoDescriptor.getElementSize(.float64_array));
    try std.testing.expectEqual(@as(u64, 8), PullIntoDescriptor.getElementSize(.bigint64_array));
    try std.testing.expectEqual(@as(u64, 8), PullIntoDescriptor.getElementSize(.biguint64_array));
}
