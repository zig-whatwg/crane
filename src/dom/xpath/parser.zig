...
test "parser - number literal" {
    ...
    try std.testing.expectEqual(PrimaryExpr.number, std.meta.activeTag(expr.primary_expr));
    try std.testing.expectEqual(@as(f64, 42.0), expr.primary_expr.number);
}
...
test "parser - string literal" {
    ...
    try std.testing.expectEqual(Expr.primary_expr, std.meta.activeTag(expr.*));
    try std.testing.expectEqual(PrimaryExpr.literal, std.meta.activeTag(expr.primary_expr));
    try std.testing.expectEqualStrings("hello", expr.primary_expr.literal);
}

test "parser - variable reference" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();

    var parser = try Parser.init(arena.allocator(), "$myVar");
    const expr = try parser.parse();

    try std.testing.expectEqual(Expr.primary_expr, std.meta.activeTag(expr.*));
    try std.testing.expectEqual(PrimaryExpr.variable_reference, std.meta.activeTag(expr.primary_expr));
    try std.testing.expectEqualStrings("$myVar", expr.primary_expr.variable_reference);
}
...
test "parser - simple arithmetic" {
    ...
    var parser = try Parser.init(arena.allocator(), "5 + 3");
    const expr = try parser.parse();

    try std.testing.expectEqual(Expr.additive_expr, std.meta.activeTag(expr.*));
    try std.testing.expectEqual(Expr.BinaryExpr.Operator.plus, expr.additive_expr.operator);
}
...
test "parser - function call" {
    ...
    var parser = try Parser.init(arena.allocator(), "position()");
    const expr = try parser.parse();
...
test "parser - simple location path" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();

    var parser = try Parser.init(arena.allocator(), "child::para");
    const expr = try parser.parse();
    ...
    try std.testing.expectEqual(@as(usize, 1), expr.location_path.relative.steps.len);
    try std.testing.expectEqual(Axis.child, expr.location_path.relative.steps[0].axis);
}

test "parser - absolute location path" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
...
test "parser - predicate" {
    ...
    var parser = try Parser.init(arena.allocator(), "child::para[position()=1]");
    const expr = try parser.parse();

    try std.testing.expectEqual(Expr.location_path, std.meta.activeTag(expr.*));
    const steps = expr.location_path.relative.steps;
    try std.testing.expectEqual(@as(usize, 1), steps.len);
    try std.testing.expectEqual(@as(usize, 1), steps[0].predicates.len);
}

test "parser - abbreviated syntax @" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    ...
    const steps = expr.location_path.relative.steps;
    try std.testing.expectEqual(@as(usize, 1), steps.len);
    try std.testing.expectEqual(Axis.attribute, steps[0].axis);
}
...
test "parser - abbreviated syntax ." {
    ...
    const steps = expr.location_path.relative.steps;
    try std.testing.expectEqual(@as(usize, 1), steps.len);
    try std.testing.expectEqual(Axis.self, steps[0].axis);
}
...
test "parser - abbreviated syntax .." {
    ...
    const steps = expr.location_path.relative.steps;
    try std.testing.expectEqual(@as(usize, 1), steps.len);
    try std.testing.expectEqual(Axis.parent, steps[0].axis);
}
...
test "parser - abbreviated syntax //" {
    ...
    var parser = try Parser.init(arena.allocator(), "//para");
    const expr = try parser.parse();

    try std.testing.expectEqual(Expr.location_path, std.meta.activeTag(expr.*));
    try std.testing.expect(expr.location_path.isAbsolute());
    const steps = expr.location_path.absolute.steps;
    // Should have descendant-or-self::node() + child::para
    try std.testing.expectEqual(@as(usize, 2), steps.len);
    try std.testing.expectEqual(Axis.descendant_or_self, steps[0].axis);
    try std.testing.expectEqual(Axis.child, steps[1].axis);
}
