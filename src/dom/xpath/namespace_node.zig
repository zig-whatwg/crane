...
test "NamespaceNode - init and deinit" {
    const allocator = testing.allocator;
    var parent: NodeBase = undefined;
    
    const node = try NamespaceNode.init(
        allocator,
        "foo",
    ...
    try testing.expectEqualStrings("foo", node.prefix);
    try testing.expectEqualStrings("http://example.com/foo", node.namespace_uri);
    try testing.expect(node.parent_element == &parent);
}

test "NamespaceNode - default namespace (empty prefix)" {
    const allocator = testing.allocator;
    var parent: NodeBase = undefined;
    ...
    try testing.expectEqualStrings("", node.prefix);
    try testing.expectEqualStrings("http://example.com/default", node.namespace_uri);
}

test "NamespaceNode - getExpandedName" {
    const allocator = testing.allocator;
    var parent: NodeBase = undefined;
    ...
    try testing.expectEqualStrings("xhtml", node.getExpandedName());
}

test "NamespaceNode - getStringValue" {
    const allocator = testing.allocator;
    var parent: NodeBase = undefined;
    ...
    try testing.expectEqualStrings("http://www.w3.org/1999/xhtml", node.getStringValue());
}

test "NamespaceNode - getParent" {
    const allocator = testing.allocator;
    var parent: NodeBase = undefined;
    ...
    try testing.expect(node.getParent() == &parent);
}
