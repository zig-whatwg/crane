...
pub const DOMTokenList = struct {
    ...
    pub fn contains(self: *const DOMTokenList, token: []const u8) bool {
        ...
        const token_u16 = infra.String.fromBytes(token) catch return false;
    ...
    pub fn add(self: *DOMTokenList, tokens: []const []const u8) !void {
        ...
        for (tokens) |token| {
            const token_u16 = try infra.String.fromBytes(token);
    ...
    pub fn replace(self: *DOMTokenList, token: []const u8, new_token: []const u8) !bool {
        ...
        const new_token_u16 = try infra.String.fromBytes(new_token);
    ...
    fn parseAndSetTokens(self: *DOMTokenList, value_str: []const u8) !void {
        ...
        const value_u16 = try infra.String.fromBytes(value_str);
        defer self.allocator.free(value_u16);

        // Split on ASCII whitespace
        const tokens = try infra.splitOnAsciiWhitespace(self.allocator, value_u16);
        defer {
            for (tokens) |token| {
                self.allocator.free(token);
            }
