...
test "euc-kr streaming decode - ASCII" {
    const input = "Hello, World!";
    var output: [50]u16 = undefined;

    var decoder = Decoder{
        .encoding = undefined,
        .state = .{ .euc_kr = .{ .euc_kr_lead = 0 } },
    };
...
test "euc-kr streaming decode - 2-byte sequence" {
    const input = [_]u8{ 0xB0, 0xA1 }; // 가 (U+AC00)
    ...
    try std.testing.expectEqual(@as(u16, 0xAC00), output[0]);
}

test "euc-kr streaming encode - ASCII" {
    const input = [_]u16{ 'H', 'e', 'l', 'l', 'o' };
    var output: [50]u8 = undefined;

    var encoder = Encoder{
        .encoding = undefined,
        .state = .{ .euc_kr = .{ .pending_high_surrogate = null } },
    };
...
test "euc-kr streaming encode - 2-byte sequence" {
    const input = [_]u16{0xAC00}; // 가
    ...
    try std.testing.expectEqualSlices(u8, &[_]u8{ 0xB0, 0xA1 }, output[0..2]);
}
