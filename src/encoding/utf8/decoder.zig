...
test "UTF-8 decode - 2-byte sequence" {
    const input = "Café"; // é is 2 bytes in UTF-8: 0xC3 0xA9
    var output: [100]u16 = undefined;
    ...
    try std.testing.expectEqual(@as(u16, 0x00E9), output[3]); // é
}

test "UTF-8 decode - 3-byte sequence" {
    const input = "世界"; // Chinese characters, 3 bytes each in UTF-8
    var output: [100]u16 = undefined;
...
test "UTF-8 decode - 4-byte sequence (emoji)" {
    const input = "💩"; // U+1F4A9, 4 bytes in UTF-8: F0 9F 92 A9
    var output: [100]u16 = undefined;
...
test "UTF-8 decode - streaming (incomplete at end)" {
    ...
    const input2 = &[_]u8{0x96}; // Final byte of 世 (U+4E16)
    ...
    try std.testing.expectEqual(@as(u16, 0x4E16), output[0]); // 世
}
