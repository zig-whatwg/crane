...
test "processQueue - basic flow" {
    const testing = std.testing;
    const allocator = testing.allocator;

    // Create a simple handler that outputs one item per input
    ...
    const TestHandler = struct {
        fn handler(ctx: *anyopaque, item: ?ByteQueue.Item, _: *ByteQueue) !HandlerResult {
            const context: *TestContext = @ptrCast(@alignCast(ctx));
            if (item) |i| {
                if (i.isEndOfQueue()) {
                    return .{ .finished = {} };
                }
                // Echo the byte as a code point (use stack buffer, no alloc)
                context.items_buffer[0] = i.value;
                return .{ .items = &context.items_buffer };
            }
            return .{ .finished = {} };
        }
    };
    ...
    const h = Handler{
        .handlerFn = TestHandler.handler,
        .context = &ctx,
    };

    // Create input queue with bytes
    var input = try ByteQueue.fromSlice(allocator, &[_]u8{ 0x41, 0x42, 0x43 });
    defer input.deinit();
    try input.markEnd();
    ...
    // Process the queue
    const result = try processQueue(h, &input, &output, .replacement);
    try testing.expectEqual(ProcessResult.finished, result);

    // Verify output
    const out_slice = try output.toSlice(allocator);
    defer allocator.free(out_slice);
    try testing.expectEqual(@as(usize, 3), out_slice.len);
    try testing.expectEqual(@as(u21, 0x41), out_slice[0]);
    try testing.expectEqual(@as(u21, 0x42), out_slice[1]);
    try testing.expectEqual(@as(u21, 0x43), out_slice[2]);
}

test "processItem - error handling replacement mode" {
    const testing = std.testing;
    const allocator = testing.allocator;

    // Handler that always returns error
    const ErrorHandler = struct {
        fn handler(ctx: *anyopaque, _: ?ByteQueue.Item, _: *ByteQueue) !HandlerResult {
            _ = ctx;
            return .{ .error_ = {} };
        }
    };
    ...
    var dummy: u8 = 0;
    const h = Handler{
        .handlerFn = ErrorHandler.handler,
        .context = &dummy,
    };

    var input = ByteQueue.init(allocator);
    defer input.deinit();
    ...
    const result = try processItem(h, null, &input, &output, .replacement);
    try testing.expectEqual(ProcessResult.continue_, result);

    // Should have pushed U+FFFD
    const out_slice = try output.toSlice(allocator);
    defer allocator.free(out_slice);
    try testing.expectEqual(@as(usize, 1), out_slice.len);
    try testing.expectEqual(@as(u21, 0xFFFD), out_slice[0]);
}

test "processItem - error handling fatal mode" {
    const testing = std.testing;
    const allocator = testing.allocator;
    ...
    // Process with fatal mode
    const result = try processItem(h, null, &input, &output, .fatal);
    try testing.expectEqual(ProcessResult.error_, result);
}
