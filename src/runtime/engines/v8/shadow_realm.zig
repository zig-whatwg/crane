...
fn shadowRealmContextCallback(
    ...
    // Use default security token - this is critical for cross-realm callable wrapping
    // per Chromium's shadow_realm_context.cc: context->UseDefaultSecurityToken()
    // Without this, wrapped functions fail when called from inside the ShadowRealm
