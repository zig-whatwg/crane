...
pub fn hydrateContextFromSnapshot(
    ...
    // NOTE: interface_bindings import removed - no longer needed with on-demand architecture
    _ = isolate; // Isolate no longer needed - templates created on-demand, not reinstalled
    ...
    inline for (std.meta.fields(helpers.GlobalScope)) |field| {
        if (scope == @field(helpers.GlobalScope, field.name)) {
            // NEW ARCHITECTURE (whatwg-izjpz): Chromium-style minimal snapshot with on-demand templates
            //
            // The snapshot now only contains 6 core DOM interfaces (EventTarget, Node, Element,
            // Document, Window). All other interfaces are created on-demand when first accessed
            // via template_registry.wrapInstanceAsV8Object() -> interface_bindings.createTemplateOnDemand().
            //
            // This eliminates the need for reinstallAllAccessorCallbacks() which was:
            // 1. Slow (iterating 1260+ interfaces)
            // 2. Broken (prototype identity bug caused MessageEvent.data to return undefined)
            //
            // With on-demand creation, templates are created fresh with all accessors correctly
            // installed from the start - no reinstallation needed.
            //
            // However, V8 snapshots don't preserve lazy data properties on the global proxy.
            ...
            std.debug.print("[HYDRATE-SNAPSHOT] Context hydrated with lazy constructors installed, scope={s}\n", .{@tagName(scope)});
            return;
        }
    }
}
...
pub fn hydrateWorkerContext(options: HydrationOptions) !WorkerHydrationResult {
    ...
    // created fresh on-demand with all accessors correctly installed from the start.
    // No reinstallation needed - eliminates the prototype identity bug.
    std.debug.print("[HYDRATE-WORKER] Using on-demand template architecture (no accessor reinstall needed)\n", .{});
