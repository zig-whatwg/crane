...
pub fn hydrateContextFromSnapshot(
    ...
    inline for (std.meta.fields(helpers.GlobalScope)) |field| {
        if (scope == @field(helpers.GlobalScope, field.name)) {
            // NOTE: The optimization from whatwg-8oip3 that skipped accessor reinstallation was WRONG.
            // V8 snapshots do NOT preserve native accessor callbacks even with external references.
            // The snapshot serializes JavaScript code but native C/Zig function pointers must be
            // reinstalled after snapshot load. Without this, accessors like MessageEvent.data
            // return undefined because the getter callback is never invoked.
            //
            // We still skip installForScope() because the interfaces ARE in the snapshot.
            // But we MUST call reinstallAllAccessorCallbacks() to restore native callbacks.
            std.debug.print("[HYDRATE-SNAPSHOT] Reinstalling accessor callbacks for scope={s}, isolate={*}, ctx={*}\n", .{ @tagName(scope), isolate, v8_ctx });
...
pub fn hydrateWindowContext(comptime namespaces_module: type, options: HydrationOptions) !WindowHydrationResult {
    ...
    // reinstalled after snapshot load. Without this, accessors like MessageEvent.data
    // return undefined because the getter callback is never invoked.
    std.debug.print("[HYDRATE-WINDOW] Reinstalling accessor callbacks after snapshot restore...\n", .{});
    interface_bindings.reinstallAllAccessorCallbacks(isolate, v8_ctx);

    // 5b. OPTIMIZATION: Also skip method callback reinstallation for same reason.
...
pub fn hydrateWorkerContext(options: HydrationOptions) !WorkerHydrationResult {
    ...
    // reinstalled after snapshot load. Without this, accessors like MessageEvent.data
    // return undefined because the getter callback is never invoked.
    std.debug.print("[HYDRATE-WORKER] Reinstalling accessor callbacks after snapshot restore...\n", .{});
    interface_bindings.reinstallAllAccessorCallbacks(isolate, v8_ctx);
