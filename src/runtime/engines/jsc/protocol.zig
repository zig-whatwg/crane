...
//! `-Dengine=jsc` build (`protocol`, below, is what the engine protocol -
...
//! destroy or collect; a value's type and identity read from its IDL arm.
//! Where the design says JSC has nothing to do - initializeEngine, and
//! performMicrotaskCheckpoint (JSC drains microtasks as its outermost call
//! returns) - that is the answer here too. The operations gated on a
...
/// listed here, when this adapter builds it on the public API.
...
/// JSC has no process-wide state to set up.
