...
const writableStream_mod = @import("writableStream");
const WritableStream = writableStream_mod.WritableStream;
/// WritableStreamDefaultWriter WebIDL interface
/// 
/// IDL:
/// ```webidl
/// [Exposed=*]
...
pub const WritableStreamDefaultWriter = struct {
    ...
    pub fn deinit(_: *WritableStreamDefaultWriter) void {
        // Promises (closedPromise, readyPromise) are allocated from the event loop's arena.
    ...
    pub fn call_write(self: *WritableStreamDefaultWriter, chunk: webidl.Optional(webidl.JSValue)) !*AsyncPromise(void) {
        ...
        const chunkValue = if (chunk.was_passed) chunk.value else webidl.JSValue{ .undefined = {} };
    ...
    pub fn writeInternal(self: *WritableStreamDefaultWriter, chunk: common.JSValue) !*AsyncPromise(void) {
        ...
        if (stream.state == .errored) {
            const promise = try AsyncPromise(void).init(self.allocator, self.eventLoop);
            promise.reject(stream.storedError orelse common.JSValue{ .string = "Stream errored" });
            return promise;
        }
        ...
        if (stream.state == .erroring) {
            const promise = try AsyncPromise(void).init(self.allocator, self.eventLoop);
            promise.reject(stream.storedError orelse common.JSValue{ .string = "Stream is erroring" });
            return promise;
        }

        // Step 10: Assert: state is "writable".
        // Step 11: Let promise be ! WritableStreamAddWriteRequest(stream).
        const promise = try stream.addWriteRequest(self.eventLoop);
    ...
    pub fn updateReadyPromise(self: *WritableStreamDefaultWriter, backpressure: bool) !void {
        if (backpressure) {
            ...
            if (self.readyPromise) |old_promise| {
                old_promise.deinit();
            }

            // Create new pending promise
            self.readyPromise = try AsyncPromise(void).init(self.allocator, self.eventLoop);
        } else {
...
// TODO: Fix test - writeAlgorithm signature changed - disabled
// TODO: Fix test - writeAlgorithm signature changed
