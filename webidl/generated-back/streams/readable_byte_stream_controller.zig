...
const ReadableStreamBYOBRequest = @import("readableStream_byobRequest").ReadableStreamBYOBRequest;
...
const PullIntoDescriptorModule = @import("pullIntoDescriptor");
...
const ReadIntoRequest = @import("readIntoRequest").ReadIntoRequest;
...
fn convertWebIDLArrayBuffer(allocator: std.mem.Allocator, webidl_buffer: webidl.ArrayBuffer) !*ArrayBuffer {
    ...
    internal_buffer.* = .{
        ...
        .byteLength = webidl_buffer.data.len,
...
/// ReadableByteStreamController WebIDL interface
/// 
/// IDL:
/// ```webidl
/// [Exposed=*]
...
pub const ReadableByteStreamController = struct {
    ...
    fn calculateDesiredSize(self: *const ReadableByteStreamController) ?f64 {
        ...
        if (self.closeRequested and self.queue.items.len == 0) {
            return 0.0;
        }
    ...
    fn enqueueInternal(self: *ReadableByteStreamController, chunk: webidl.ArrayBufferView) !void {
        ...
        if (self.pendingPullIntos.items.len > 0) {
            ...
            const first_pending_pull_into = self.pendingPullIntos.items[0];
        ...
        if (stream.hasDefaultReader()) {
            ...
            if (stream.getNumReadRequests() == 0) {
                ...
                try self.enqueueChunkToQueue(transferred_buffer_ptr, byteOffset, byteLength);
            } else {
                // Step 9.3: Otherwise,
    ...
    fn getBYOBRequest(self: *ReadableByteStreamController) !?*ReadableStreamBYOBRequest {
        ...
        if (self.byobRequest == null and self.pendingPullIntos.items.len > 0) {
            ...
            const view_byteOffset = firstDescriptor.byteOffset + firstDescriptor.bytes_filled;
            ...
            const webidl_buffer = try convertToWebIDLArrayBuffer(self.allocator, firstDescriptor.buffer);
            ...
            const uint8_view = try Uint8ArrayType.init(
                webidl_buffer,
                view_byteOffset,
                view_byteLength, // For Uint8Array, length in elements = length in bytes
            );
            ...
            byobRequest.* = ReadableStreamBYOBRequest.init(
                self.allocator,
                self,
                view,
            );
    ...
    fn fillPullIntoDescriptorFromQueue(
        ...
        const remaining_space = pullIntoDescriptor.byteLength - pullIntoDescriptor.bytes_filled;
        ...
        while (total_bytes_to_copy_remaining > 0) {
            ...
            const dest_start = pullIntoDescriptor.byteOffset + pullIntoDescriptor.bytes_filled;
            ...
            const src_slice = queue_buffer.data[queue_byteOffset..][0..bytes_to_copy];
    ...
    fn fillReadRequestFromQueue(self: *ReadableByteStreamController) !void {
        ...
        if (self.stream) |stream| {
            ...
            const chunkValue = common.JSValue{ .object = {} };
            stream.fulfillReadRequest(chunkValue, false);
        }
    }
    ...
    pub fn pullInto(
        ...
        pullIntoDescriptor.* = .{
            .buffer = buffer_ptr,
            .buffer_byteLength = buffer_ptr.data.len,
            .byteOffset = byteOffset,
            .byteLength = byteLength,
            .bytes_filled = 0,
        ...
        if (stream.state == .closed) {
            ...
            const empty_view = try view_construction.constructView(ctor, buffer_ptr, byteOffset, 0);
    ...
    fn respondWithNewView(self: *ReadableByteStreamController, view: webidl.ArrayBufferView) !void {
        ...
        firstDescriptor.buffer.* = transferred_buffer;
        // Clean up temporary buffer
    ...
    fn respond(self: *ReadableByteStreamController, bytesWritten: u64) !void {
        // Step 1: Assert: controller.[[pendingPullIntos]] is not empty.
        if (self.pendingPullIntos.items.len == 0) {
            return error.InvalidState;
        }
        ...
        if (state == .closed) {
            ...
            if (firstDescriptor.bytes_filled + bytesWritten > firstDescriptor.byteLength) {
                return error.RangeError;
            }
        }

        // Step 6: Set firstDescriptor's buffer to ! TransferArrayBuffer(firstDescriptor's buffer).
        const old_descriptor_buffer = firstDescriptor.buffer;
        ...
        firstDescriptor.buffer = transferred_desc_ptr;
    ...
    fn respondInternal(self: *ReadableByteStreamController, bytesWritten: u64) !void {
        ...
        if (state == .closed) {
            ...
            try self.respondInClosedState(firstDescriptor);
        } else {
            // Step 6: Otherwise,
            // Step 6.1: Assert: state is "readable".
    ...
    fn respondInClosedState(
        ...
        if (stream.hasBYOBReader()) {
            ...
            while (filled_pull_intos.items.len < stream.getNumReadIntoRequests()) {
                ...
                const pullIntoDescriptor = self.shiftPendingPullInto();
    ...
    fn respondInReadableState(
        ...
        if (remainder_size > 0) {
            ...
            const end = pullIntoDescriptor.byteOffset + pullIntoDescriptor.bytes_filled;
