...
const ReadableStreamDefaultController = @import("readableStream_default_controller").ReadableStreamDefaultController;
const ReadableStreamDefaultReader = @import("readableStream_default_reader").ReadableStreamDefaultReader;
const ReadableStreamBYOBReader = @import("readableStream_byob_reader").ReadableStreamBYOBReader;
const WritableStream = @import("writableStream").WritableStream;
const WritableStreamDefaultWriter = @import("writableStream_default_writer").WritableStreamDefaultWriter;
...
pub const PipeState = struct {
    ...
    chainedRead: ?*AsyncPromise(common.ReadResult),
    chainedWrite: ?*AsyncPromise(void),
    ...
    pub fn deinit(self: *PipeState) void {
        ...
        if (self.chainedRead) |p| {
            p.deinit();
            self.chainedRead = null;
        }
        if (self.chainedWrite) |p| {
            p.deinit();
            self.chainedWrite = null;
        }
    ...
    fn shuttleNext(self: *PipeState) !void {
        ...
        if (self.source.state == .errored) {
            ...
            const err = self.source.storedError orelse common.JSValue{ .string = "Source errored" };
            if (!self.preventAbort) {
                // Abort destination and shutdown
                self.shutdownWithAction(.{ .abortDestination = err }, err);
            } else {
                self.shutdown(err);
            }
            return;
        ...
        if (self.dest.state == .errored) {
            ...
            const err = self.dest.storedError orelse common.JSValue{ .string = "Destination errored" };
            if (!self.preventCancel) {
                // Cancel source and shutdown
                self.shutdownWithAction(.{ .cancelSource = err }, err);
            } else {
                self.shutdown(err);
            }
            return;
    ...
    fn onChunkRead(ctx: *anyopaque, result: common.ReadResult) !common.ReadResult {
        ...
        if (result.done) {
            ...
            if (!self.preventClose) {
                ...
                self.shutdownWithAction(.closeWriter, null);
            } else {
                self.shutdown(null);
            }
            return result;
        ...
        if (result.value) |chunk| {
            ...
            self.pendingWrite = write_promise;
    ...
    fn onReadError(ctx: *anyopaque, error_value: common.JSValue) !common.ReadResult {
        ...
        if (!self.preventAbort) {
            ...
            self.shutdownWithAction(.{ .abortDestination = error_value }, error_value);
        } else {
    ...
    fn onWriteError(ctx: *anyopaque, error_value: common.JSValue) !void {
        ...
        if (!self.preventCancel) {
            ...
            self.shutdownWithAction(.{ .cancelSource = error_value }, error_value);
        } else {
            self.shutdown(error_value);
        }
    }
    ...
    const ShutdownAction = union(enum) {
        abortDestination: ?common.JSValue,
        cancelSource: ?common.JSValue,
        closeWriter: void,
    };
...
pub const ReadableStreamIterator = struct {
    ...
    pub fn next(self: *ReadableStreamIterator) !?common.JSValue {
        ...
        if (read_promise.isFulfilled()) {
            ...
            defer read_promise.deinit(); // Safe to free - promise was fulfilled and removed from readRequests
            if (result.done) {
                self.done = true;
                return null;
            }
            ...
            defer read_promise.deinit(); // Safe to free - promise was rejected and removed from readRequests
            self.done = true;
            return error.StreamError;
        } else {
            // Still pending - DO NOT call deinit() because the promise is still in readRequests
...
pub const TeeState = struct {
    ...
    cancelPromiseFulfilled: bool,

    // Clone flag
    cloneForBranch2: bool,

    // Read promise from last pullAlgorithm call (for cleanup)
    lastReadPromise: ?*AsyncPromise(common.ReadResult),

    // Chained promise from thenCtx() call (for cleanup)
    chainedPromise: ?*AsyncPromise(common.ReadResult),

    // Reference counting for shared ownership
    refCount: usize,

    // Algorithm wrappers (owned by TeeState, freed when refCount reaches 0)
    pullWrapper: *TeeStatePullAlgorithm,
    cancel1Wrapper: *TeeStateCancelAlgorithm,
    cancel2Wrapper: *TeeStateCancelAlgorithm,
    ...
    pub fn pullAlgorithm(self: *TeeState) common.Promise(void) {
        ...
        if (self.lastReadPromise) |old| old.deinit();
        self.lastReadPromise = read_promise;
    ...
    fn onChunkRead(ctx: *anyopaque, result: common.ReadResult) !common.ReadResult {
        ...
        if (result.done) {
            ...
            if (!self.canceled1 or !self.canceled2) {
                self.cancelPromiseFulfilled = true;
            }
            ...
            self.eventLoop.queueMicrotask(.{
                .callback = struct {
                    fn distribute(microtask_ctx: ?*anyopaque) void {
                        ...
                        const chunk_val = dist.chunkValue;
                        ...
                        if (!state.canceled2 and state.cloneForBranch2) {
    ...
    pub fn cancel1Algorithm(self: *TeeState, reason: ?common.JSValue) common.Promise(void) {
        ...
        if (self.cancelPromiseFulfilled) {
            return common.Promise(void).fulfilled({});
        } else {
    ...
    pub fn release(self: *TeeState) void {
        ...
        if (self.refCount == 0) {
            ...
            if (self.lastReadPromise) |promise| {
                promise.deinit();
            }
            if (self.chainedPromise) |promise| {
                promise.deinit();
            }

            // Free algorithm wrappers
            self.allocator.destroy(self.pullWrapper);
            self.allocator.destroy(self.cancel1Wrapper);
            self.allocator.destroy(self.cancel2Wrapper);
...
pub const TeeStatePullAlgorithm = struct {
    ...
    fn call(ctx: *anyopaque) common.Promise(void) {
        ...
        return self.teeState.pullAlgorithm();
    }
};
...
pub const TeeStateCancelAlgorithm = struct {
    ...
    fn call(ctx: *anyopaque, reason: ?common.JSValue) common.Promise(void) {
        ...
        return if (self.branch == 0)
            self.teeState.cancel1Algorithm(reason)
        else
            self.teeState.cancel2Algorithm(reason);
    }
};
...
/// ReadableStream WebIDL interface
/// 
/// IDL:
/// ```webidl
...
pub const ReadableStream = struct {
    ...
    pub fn deinit(self: *ReadableStream) void {
        ...
        //
        // If the event loop is owned by this stream (eventLoop_storage != null),
        // all promises are allocated from the event loop's arena and will be freed
        // when the arena is destroyed.
        //
        // If the event loop is borrowed (eventLoop_storage == null), the caller
    ...
    pub fn initWithSource(
        ...
        // For now, start callbacks are not supported (GenericCallback cannot be invoked yet)
        ...
        // TODO: Start callback invocation not yet supported - requires zig-js-runtime
        // When GenericCallback invocation is available, uncomment:
        // if (source_dict.start) |start_fn| {
        //     const start_result = start_fn.vtable.invoke(...);
    ...
    pub fn iterator(
        ...
        return ReadableStreamIterator.init(allocator, self, loop, preventCancel);
    }
    ...
    pub fn call_pipeThrough(
        ...
        const WS = @import("writableStream").WritableStream;
        const writable = try webidl.unwrapInterface(WS, writable_value);

        // Parse options if provided
        var preventClose: bool = false;
        var preventAbort: bool = false;
        var preventCancel: bool = false;
        ...
        if (options.was_passed) {
            ...
            if (opts == .object) {
                ...
                if (opts_obj.get("preventClose")) |pc| {
                    preventClose = pc.toBoolean();
                }
                if (opts_obj.get("preventAbort")) |pa| {
                    preventAbort = pa.toBoolean();
                }
                if (opts_obj.get("preventCancel")) |pc| {
                    preventCancel = pc.toBoolean();
                }
    ...
    pub fn getNumReadRequests(self: *const ReadableStream) usize {
        switch (self.reader) {
            .none => return 0,
            .default => |reader| return reader.readRequests.items.len,
    ...
    pub fn getNumReadIntoRequests(self: *const ReadableStream) usize {
        switch (self.reader) {
            ...
            .byob => |reader| return reader.readIntoRequests.items.len,
        }
    }
    ...
    fn cancelInternal(self: *ReadableStream, reason: ?common.JSValue) common.Promise(void) {
        ...
        if (self.state == .errored) {
            return common.Promise(void).rejected(self.storedError orelse common.JSValue.undefined_value());
        }
    ...
    pub fn acquireDefaultReader(self: *ReadableStream, loop: eventLoop.EventLoop) !*ReadableStreamDefaultReader {
    ...
    pub fn acquireBYOBReader(self: *ReadableStream, loop: eventLoop.EventLoop) !*ReadableStreamBYOBReader {
        ...
        switch (self.state) {
            ...
            .errored => {
                return common.Promise(common.ReadResult).rejected(
                    self.storedError orelse common.JSValue.undefined_value(),
                );
            },
            ...
            .readable => {
                ...
                if (self.controller.queue.isEmpty()) {
                    ...
                    const chunkValue: common.JSValue = queue_value;
    ...
    fn teeInternal(self: *ReadableStream, cloneForBranch2: bool) ![2]*ReadableStream {
        ...
        return self.teeDefault(cloneForBranch2);
    }
    ...
    fn teeDefault(self: *ReadableStream, cloneForBranch2: bool) ![2]*ReadableStream {
        ...
        const pullWrapper = try self.allocator.create(TeeStatePullAlgorithm);
        errdefer self.allocator.destroy(pullWrapper);

        // Each branch has its OWN cancel algorithm with branch index
        const cancel1Wrapper = try self.allocator.create(TeeStateCancelAlgorithm);
        errdefer self.allocator.destroy(cancel1Wrapper);

        const cancel2Wrapper = try self.allocator.create(TeeStateCancelAlgorithm);
        errdefer self.allocator.destroy(cancel2Wrapper);

        // Create shared TeeState
        const teeState = try self.allocator.create(TeeState);
        errdefer self.allocator.destroy(teeState);

        // Initialize TeeState (steps 4-12)
        // Step 12: cancelPromise simplified to a boolean flag (no event loop needed)
        // refCount starts at 2 (one for each branch)
        teeState.* = TeeState{
            ...
            .cancelPromiseFulfilled = false,
            .cloneForBranch2 = cloneForBranch2,
            .lastReadPromise = null,
            .chainedPromise = null,
            .refCount = 2, // Both branches hold references
            .pullWrapper = pullWrapper,
            .cancel1Wrapper = cancel1Wrapper,
            .cancel2Wrapper = cancel2Wrapper,
        };

        // Now initialize the wrappers with the TeeState pointer
        pullWrapper.* = .{ .teeState = teeState };
        const pullAlgorithm = pullWrapper.algorithm();

        cancel1Wrapper.* = .{ .teeState = teeState, .branch = 0 };
        const cancel1_algorithm = cancel1Wrapper.algorithm();

        cancel2Wrapper.* = .{ .teeState = teeState, .branch = 1 };
        const cancel2_algorithm = cancel2Wrapper.algorithm();
        ...
        branch1.* = try ReadableStream.initWithSource(self.allocator, self.eventLoop, null, null);
        ...
        branch1.controller.cancelAlgorithm = cancel1_algorithm;
        branch1.controller.pullAlgorithm = pullAlgorithm; // Shared!
        branch1.controller.strategySizeAlgorithm = size_algorithm;
        ...
        branch2.* = try ReadableStream.initWithSource(self.allocator, self.eventLoop, null, null);
        ...
        branch2.controller.cancelAlgorithm = cancel2_algorithm; // Different!
        branch2.controller.pullAlgorithm = pullAlgorithm; // Same as branch1!
        branch2.controller.strategySizeAlgorithm = size_algorithm;
...
test "ReadableStream - read returns error when stream is errored" {
    ...
    stream.storedError = common.JSValue{ .string = "test error" };
...
test "ReadableStream - fulfill read request directly" {
    ...
    // Manually create and add a promise to readRequests
    const promise = try AsyncPromise(common.ReadResult).init(allocator, loop.eventLoop());
    try reader.readRequests.append(allocator, promise);
    ...
    // Promise should be removed from readRequests
...
test "ReadableStream - tee creates shared TeeState" {
    ...
    const branches = try stream.teeInternal(false);
    const teeState = branches[0].teeState.?;
    ...
    try std.testing.expect(branches[0].teeState != null);
    try std.testing.expect(branches[1].teeState != null);

    // Both branches should point to the SAME TeeState
    try std.testing.expect(branches[0].teeState == branches[1].teeState);

    // TeeState should have coordination flags initialized
    try std.testing.expect(!teeState.reading);
    try std.testing.expect(!teeState.readAgain);
    try std.testing.expect(!teeState.canceled1);
    try std.testing.expect(!teeState.canceled2);
    try std.testing.expect(teeState.refCount == 2); // Both branches hold references
}
...
test "ReadableStream - tee cancel coordination (both branches must cancel)" {
    ...
    // Cancel branch 1
    _ = teeState.cancel1Algorithm(common.JSValue{ .string = "reason1" });

    // TeeState should show branch1 canceled but cancelPromise NOT fulfilled
    try std.testing.expect(teeState.canceled1);
    try std.testing.expect(!teeState.canceled2);
    try std.testing.expect(!teeState.cancelPromiseFulfilled);

    // Cancel branch 2
    _ = teeState.cancel2Algorithm(common.JSValue{ .string = "reason2" });
...
test "ReadableStream - tee pull coordination reads and distributes chunks" {
    ...
    const pull_result = teeState.pullAlgorithm();
    ...
    teeState.eventLoop.runMicrotasks();
    ...
    // Read from branch 1
    // We can use teeState.eventLoop (it's now the stream's event loop, not a temp one)
...
test "ReadableStream - tee pull coordination handles readAgain flag" {
    ...
    const pull_result2 = teeState.pullAlgorithm();
...
test "ReadableStream - iterator with preventCancel" {
    const allocator = std.testing.allocator;
    ...
    // Create iterator with preventCancel = true
    ...
    // doesn't cancel the stream when preventCancel is true
}
...
test "ReadableStream.from() - empty array creates closed stream" {
    ...
    try std.testing.expectEqual(@as(f64, 0.0), stream.controller.queue.queueTotalSize);
}
...
test "ReadableStream - tee branches share pull algorithm" {
    ...
    try std.testing.expectEqual(branch1.controller.pullAlgorithm.ptr, branch2.controller.pullAlgorithm.ptr);
    try std.testing.expectEqual(branch1.controller.pullAlgorithm.vtable, branch2.controller.pullAlgorithm.vtable);
}
...
test "ReadableStream - tee branches have different cancel algorithms" {
    ...
    try std.testing.expect(branch1.controller.cancelAlgorithm.ptr != branch2.controller.cancelAlgorithm.ptr);
}
...
test "ReadableStream - tee pull coordination sets reading flag" {
    ...
    const pull_result = branch1.controller.pullAlgorithm.call();
    try std.testing.expect(pull_result.isFulfilled());
}
...
test "ReadableStream - tee cancel coordination tracks both branches" {
    ...
    // Cancel branch1
    const cancel1_result = branch1.controller.cancelAlgorithm.call(.{ .string = "reason1" });
    try std.testing.expect(cancel1_result.isPending()); // Should be pending until both cancel

    // Cancel branch2
    const cancel2_result = branch2.controller.cancelAlgorithm.call(.{ .string = "reason2" });
...
test "ReadableStream - tee with cloning flag" {
    ...
    const branches = try source.teeInternal(true); // cloneForBranch2 = true
    const branch1 = branches[0];
    const branch2 = branches[1];
    defer {
        branch1.deinit();
    ...
    // The cloneForBranch2 flag is stored in TeeState and used during chunk distribution
...
test "ReadableStream - queue drainage on close with pending reads" {
    ...
    try std.testing.expect(stream.controller.closeRequested);
