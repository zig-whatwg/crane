...
//! const text = "Hello, 世界!";
//! const bytes = try encoder.call_encode(text);
//! defer allocator.free(bytes);
//! // bytes is UTF-8 encoded
//!
//! // Encode into existing buffer (zero-copy)
//! var buffer: [100]u8 = undefined;
//! const result = try encoder.call_encodeInto(text, &buffer);
...
//!
//! // Encode
//! const utf8_output = try encoder.call_encode(utf8_input);
...
//! ```zig
//! var encoder = TextEncoder.init(allocator);
//! const bytes = try encoder.call_encode("Hello");
//! defer allocator.free(bytes);
//! ```
//!
//! ### Zero-Copy Encode
//! ```zig
//! var buffer: [100]u8 = undefined;
//! const result = try encoder.call_encodeInto("Hello, 世界!", &buffer);
...
pub const TextEncoder = webidl.interface(struct {
    ...
    /// ```zig
    /// const bytes = try encoder.call_encode("世界");
    /// defer allocator.free(bytes);
    ...
    /// ### Empty String
    /// ```zig
    /// const bytes = try encoder.call_encode("");
    /// defer allocator.free(bytes);
    /// // bytes.len == 0
    /// ```
    ///
