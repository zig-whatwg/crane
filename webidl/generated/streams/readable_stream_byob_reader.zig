...
const ReadIntoRequestModule = @import("readIntoRequest");
...
/// ReadableStreamBYOBReader WebIDL interface
/// 
/// IDL:
/// ```webidl
/// [Exposed=*]
...
pub const ReadableStreamBYOBReader = struct {
    ...
    readIntoRequests: std.ArrayList(ReadIntoRequest),
    ...
    fn setupInternal(self: *ReadableStreamBYOBReader, stream: *ReadableStream) !void {
        const RS = @import("readableStream").ReadableStream;
    ...
    fn readInternal(
        ...
        const PromiseContext = struct {
            ...
            viewRef: webidl.ArrayBufferView,
    ...
    fn genericInitialize(self: *ReadableStreamBYOBReader, stream: anytype) !void {
        ...
        const RS = @import("readableStream");
        ...
        if (stream.state == .readable) {
            ...
            self.closedPromise = try AsyncPromise(void).init(self.allocator, self.eventLoop);
        }
    ...
    fn genericRelease(self: *ReadableStreamBYOBReader) void {
        ...
        // Step 5: Otherwise, set reader.[[closedPromise]] to a promise rejected with a TypeError exception.
            ...
            const new_promise = AsyncPromise(void).init(self.allocator, self.eventLoop) catch return;
            new_promise.reject(common.JSValue{ .string = "Reader released" });
            self.closedPromise = new_promise;
        }
        ...
        const RS_module = @import("readableStream");
    ...
    fn errorReadIntoRequests(self: *ReadableStreamBYOBReader, e: common.JSValue) void {
        ...
        const readIntoRequests = self.readIntoRequests.items;

        // Step 2: Set reader.[[readIntoRequests]] to a new empty list.
        self.readIntoRequests.clearRetainingCapacity();

        // Step 3: For each readIntoRequest of readIntoRequests,
        //         perform readIntoRequest's error steps, given e.
        const ReadIntoReq = @import("readIntoRequest");
        for (readIntoRequests) |request| {
            const err_value = switch (e) {
...
test "ReadableStreamBYOBReader - basic construction" {
    ...
    try std.testing.expect(reader.closedPromise != null);
