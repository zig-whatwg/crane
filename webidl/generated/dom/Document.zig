...
pub const Document = struct {
    ...
    pub const node_type_VALUE = Node.DOCUMENT_NODE;
            ...
            fn findById(nodes: anytype, target_id: []const u8) ?*Element {
                for (nodes) |node| {
                    ...
                    if (node.node_type == NodeType.ELEMENT_NODE) {
                        ...
                        const attributes = try element.get_attributes();
                        if (try attributes.call_getNamedItem("id")) |id_attr| {
    ...
    pub fn asConst(self: *const Document, comptime T: type) ?*const T {
        comptime {
            if (!@hasDecl(T, "node_type_VALUE")) {
                @compileError("Cannot cast to " ++ @typeName(T) ++ ": type must declare 'pub const node_type_VALUE'");
            }
