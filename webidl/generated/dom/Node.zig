...
pub const Node = struct {
    ...
    fn ensureEventListenerList(self: *Node) !*infra.List(EventListener) {
    ...
    /// Convert Node to CharacterData if it's a text-like node
    pub fn asCharacterData(self: *const Node) ?*const CharacterData {
        return switch (self.node_type) {
            Node.TEXT_NODE, Node.COMMENT_NODE, Node.PROCESSING_INSTRUCTION_NODE, Node.CDATA_SECTION_NODE => @ptrCast(self),
            else => null,
        };
    }

    /// Convert Node to Element if it's an element node
    pub fn asElement(self: *const Node) ?*const Element {
        return switch (self.node_type) {
            Node.ELEMENT_NODE => @ptrCast(self),
            else => null,
        };
    }

    /// Convert Node to Document if it's a document node
    pub fn asDocument(self: *const Node) ?*const Document {
        return switch (self.node_type) {
            Node.DOCUMENT_NODE => @ptrCast(self),
            else => null,
        };
    }
};
