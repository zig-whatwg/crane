...
//! - **countMap**: OrderedMap of labels to counts (for count/countReset)
//! - **timerTable**: OrderedMap of labels to start times (for time/timeLog/timeEnd)
...
pub const Console = struct {
    ...
    /// Frees all allocated memory for countMap, timerTable, groupStack, messageBuffer, and labelPool.
    ...
    fn logger(self: *Console, logLevel: LogLevel, args: []const webidl.JSValue) void {
    ...
    fn printer(self: *Console, logLevel: LogLevel, args: []const webidl.JSValue) void {
        self.printerWithOwnedStrings(logLevel, args, &.{});
    }
    ...
    fn printerWithOwnedStrings(
        ...
        ownedStrings: []const []const u8,
    ) void {
        // Capture current indentation level
        const indent = self.groupStack.items_list.size();
        ...
        // Set printFn to null for production mode (buffer only)
        if (self.printFn) |print| {
    ...
    pub fn call_assert(self: *Console, condition: webidl.boolean, data: []const webidl.JSValue) void {
        ...
        if (first != .string) {
            ...
            const ownedStrings = [_][]const u8{concat};
            ...
            if (modified_data.len == 1) {
                ...
                self.printerWithOwnedStrings(.assert_level, modified_data, &ownedStrings);
            } else {
    ...
    pub fn call_clear(self: *Console) void {
        ...
        while (self.groupStack.pop()) |popped_group| {
    ...
    pub fn call_trace(self: *Console, data: []const webidl.JSValue) void {
        ...
        if (self.runtime) |rt| {
            ...
            defer {
                for (frames) |frame| {
                    if (frame.functionName) |name| self.allocator.free(name);
                    if (frame.fileName) |file| self.allocator.free(file);
                }
    ...
    pub fn call_count(self: *Console, label: webidl.DOMString) !void {
        ...
        const current_count = self.countMap.get(label) orelse 0;
        const new_count = current_count + 1;
        try self.countMap.set(label, new_count);
    ...
    pub fn call_countReset(self: *Console, label: webidl.DOMString) !void {
        ...
        if (self.countMap.contains(label)) {
            try self.countMap.set(label, 0);
        } else {
    ...
    pub fn call_group(self: *Console, data: []const webidl.JSValue) !void {
        ...
        try self.groupStack.push(group);
    }
    /// groupCollapsed(...data)
    /// 
    ...
    pub fn call_groupEnd(self: *Console) void {
        ...
        if (self.groupStack.pop()) |popped_group| {
            // Free the label if it was allocated
            if (popped_group.label) |lbl| {
                self.allocator.free(lbl);
            }
    ...
    pub fn call_time(self: *Console, label: webidl.DOMString) !void {
        ...
        if (self.timerTable.contains(label)) {
        ...
        try self.timerTable.set(label, now);
    }
    /// timeLog(label, ...data)
    /// 
    ...
    pub fn call_timeLog(self: *Console, label: webidl.DOMString, data: []const webidl.JSValue) !void {
        ...
        const start_time = self.timerTable.get(label) orelse {
    ...
    pub fn call_timeEnd(self: *Console, label: webidl.DOMString) !void {
        ...
        _ = self.timerTable.remove(label);
...
test "Console.clear - removes messages and empties groups" {
    ...
    try testing.expect(console.messageBuffer.size() > 0);
    try testing.expect(console.groupStack.items_list.size() > 0);
    ...
    try testing.expectEqual(@as(usize, 0), console.groupStack.items_list.size());
}
...
test "Console.table - falls back to logging" {
    ...
    const msg = console.messageBuffer.get(0).?;
    try testing.expectEqual(LogLevel.log, msg.logLevel);
